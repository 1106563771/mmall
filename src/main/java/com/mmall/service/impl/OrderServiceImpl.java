...
public class OrderServiceImpl implements IOrderService {
    ...
    private ServerResponse getCartOrderItem(Integer userId, List<Cart> cartList) {
        ...
        for (Cart cartItem : cartList) {
            ...
            if (Const.ProductStatusEnum.ON_SALE.getCode() != product.getStatus()) {
                return ServerResponse.createByErrorMessage("产品目前已经下架或者删除，不是在线售卖状态");
            }
            //校验库存
    ...
    public ServerResponse pay(Long orderNo, Integer userId, String path) {
        ...
        // 创建一个订单商品列表信息，参数含义分别为商品id（使用国标）、名称、单价（单位为分）、数量，如果需要添加商品类别，详见GoodsDetail
        List<OrderItem> orderItemList = orderItemMapper.getByOrderNoUserId(orderNo, userId);
        for (OrderItem orderItem : orderItemList) {
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                ZxingUtils.getQRCodeImge(response.getQrCode(), 265, qrPath);
    ...
    public ServerResponse aliCallback(Map<String, String> params) {
        ...
        if (order == null) {
            return ServerResponse.createByErrorMessage("非快乐mmall商城的订单，回调忽略");
        }
